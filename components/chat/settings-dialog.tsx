  ...
  return (
    <div className="dialog-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <section className="settings-dialog" role="dialog" aria-modal="true" aria-labelledby="settings-title">
        ...
        <div className="setting-row"><div><strong>Show thinking</strong><small>Display the model&apos;s reasoning in a collapsible panel.</small></div><button className={`toggle ${settings.showThinking ? "on" : ""}`} role="switch" aria-checked={settings.showThinking} onClick={() => onChange({ ...settings, showThinking: !settings.showThinking })}><span /></button></div>
